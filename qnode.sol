...
contract QNodeKey is ERC721Drop {
    ...
    event NodePurchased(address indexed buyer, uint256 quantity, address paymentToken, string referrerCode);
    event Withdrawn(address indexed owner, uint256 amount);
    event TokenWithdrawn(address indexed owner, address token, uint256 amount);
    event DebugReferral(address indexed referrer, uint256 totalReward, uint256 remainingQuantity);
    event DebugPriceCalculation(uint256 pricePerNode, uint256 discountPerNode, uint256 totalPrice, uint256 remainingQuantity);
    event DebugTokenTransfer(uint256 allowance, uint256 balance, uint256 totalPrice);
    constructor(
    ...
    tierSizes[1] = 5000; tierPrices[1] = 0.00007 ether;
    tierSizes[2] = 5000; tierPrices[2] = 0.0000875 ether;
    tierSizes[3] = 4750; tierPrices[3] = 0.0001094 ether;
    tierSizes[4] = 4500; tierPrices[4] = 0.0001367 ether;
    tierSizes[5] = 4250; tierPrices[5] = 0.0001708 ether;
    tierSizes[6] = 4000; tierPrices[6] = 0.0002 ether; // Random size and price for Tier 6
    tierSizes[7] = 3750; tierPrices[7] = 0.0002669 ether;
    tierSizes[8] = 3500; tierPrices[8] = 0.0003337 ether;
    tierSizes[9] = 3250; tierPrices[9] = 0.0004171 ether;
    tierSizes[10] = 3000; tierPrices[10] = 0.0005214 ether;
    tierSizes[11] = 2800; tierPrices[11] = 0.0006518 ether;
    tierSizes[12] = 2600; tierPrices[12] = 0.0008148 ether;
    ...
    function setBaseImageURI(string memory _newBaseImageURI) external onlyOwner {
        baseImageURI = _newBaseImageURI;
        emit BaseImageURISet(_newBaseImageURI);
    }
    ...
    function setMaxTiers(uint256 newMaxTiers, uint256[] memory newTierPrices, uint256[] memory newTierSizes) external onlyOwner {
        require(newMaxTiers >= currentTier, "New max tiers cannot be less than the current tier");
        require(newTierPrices.length == newTierSizes.length, "Prices and sizes length mismatch");
        require(newTierPrices.length <= newMaxTiers - MAX_TIERS, "Exceeds allowed new tiers");
    ...
    function generateReferralCode(address user) public pure returns (string memory) {
        ...
        // Convert the hash to a string and return it as the referral code
    ...
    function getReferralRewards(address user, address token) external view returns (uint256) {
    ...
    if (bytes(referrerCode).length > 0) {
        ...
        while (remainingQuantity > 0) {
            ...
            emit DebugPriceCalculation(pricePerNode, discountPerNode, totalPrice, remainingQuantity);

            if (remainingQuantity <= availableInCurrentTier) {
                totalPrice += (pricePerNode - discountPerNode) * remainingQuantity;
                referralRewards[referrer][paymentToken] += rewardPerNode * remainingQuantity;
                tierMinted += remainingQuantity;
                remainingQuantity = 0;
            } else {
                totalPrice += (pricePerNode - discountPerNode) * availableInCurrentTier;
                referralRewards[referrer][paymentToken] += rewardPerNode * availableInCurrentTier;
                remainingQuantity -= availableInCurrentTier;
                _advanceTier();
            }

            emit DebugReferral(referrer, referralRewards[referrer][paymentToken], remainingQuantity);
        }
    } else {
        while (remainingQuantity > 0) {
            uint256 pricePerNode = getTokenPrice(paymentToken);
            ...
            emit DebugPriceCalculation(pricePerNode, 0, totalPrice, remainingQuantity);
    ...
    emit DebugPriceCalculation(0, 0, totalPrice, quantity); // Final total price
    ...
    if (paymentToken == address(0)) {
        ...
        emit DebugTokenTransfer(allowance, balance, totalPrice);

        require(allowance >= totalPrice, "Insufficient token allowance");
        require(balance >= totalPrice, "Insufficient token balance");
    ...
    function withdrawReferralReward(address token) external {
        uint256 reward = referralRewards[msg.sender][token];
        require(reward > 0, "No rewards to withdraw");

        referralRewards[msg.sender][token] = 0; // Reset rewards
        ...
        if (token == address(0)) {
            ...
            // Withdraw in ERC20 tokens
            IERC20 erc20 = IERC20(token);
            require(erc20.balanceOf(address(this)) >= reward, "Insufficient token balance");
            require(erc20.transfer(msg.sender, reward), "Token transfer failed");
        }
    }
